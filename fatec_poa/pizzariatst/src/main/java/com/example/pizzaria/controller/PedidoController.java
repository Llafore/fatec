...
public class PedidoController {
    ...
    public ResponseEntity<Pedido> post(@RequestBody Pedido pedido) {
        // Para cada item do menu, verificar se já foi salvo e, caso não, salvar no banco de dados
        for (Menu menuItem : pedido.getItem()) {
            if (menuItem.getId() == 0) { // Se o item não tiver ID, significa que não foi salvo
                menuRepository.save(menuItem); // Salva o item no banco
            }
        }
        // Agora que os itens do Menu foram salvos, salva o Pedido
